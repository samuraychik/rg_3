class_name CardSymbolClubs extends CardSymbol


@onready var clubs_animator: AnimationPlayer = $ClubsAnimator
@onready var hit_final_sfx: AudioStreamPlayer = $HitFinalSfx

var hit_timings: Array[float] = [0.25]
...
func cue() -> void:
  cue_sfx.play()
  clubs_animator.stop(true)
  clubs_animator.play("cue")


func hit() -> void:
  hit_sfx.play()
  clubs_animator.stop(true)
  clubs_animator.play("hit")


func hit_final() -> void:
  hit_final_sfx.play()
  clubs_animator.stop(true)
  clubs_animator.play("hit_final")
